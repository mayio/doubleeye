...
int main(int argc, char** argv) {
  if (argc < 5) {
    std::fprintf(stderr,
        "usage: %s LEFT.y8 RIGHT.y8 W H [options]\n"
        "\n"
        "  Dense stereo on a raw 8-bit pair. Writes W*H float32 disparity with\n"
        "  --out; NaN means the matcher declined to answer.\n"
        "\n"
        "GEOMETRY\n"
        "  --dmax N          disparities searched, 1..N        (default 60)\n"
        "  --threads N       workers, 0 = hardware concurrency (default 0)\n"
        "\n"
        "WHAT TO TRUST (these move the precision/coverage trade)\n"
        "  --min-margin F    reject a match whose best-minus-second score is below\n"
        ...
        "                    score distribution. Above zero they act as a second\n"
        "                    margin gate.\n"
        "\n"
        "COST AND AGGREGATION\n"
        "  --ad F            weight of the truncated absolute difference blended\n"
        "                    into the Census score. Helps selection, hurts the\n"
        "                    sub-pixel fit                          (default 0.15)\n"
        "  --ad-trunc N      where that difference saturates, 8-bit  (default 10)\n"
        "  --sigma-s F       aggregation reach. Swept: 6-8 is the optimum on both\n"
        "                    axes, 12 was past it                    (default 8)\n"
        "  --sigma-r F       edge sensitivity of the filter          (default 0.2)\n"
        "  --csct            24-bit centre-symmetric Census instead of 48-bit.\n"
        "                    Trades 3.7 points of coverage for 0.5 of error\n"
        "  --agg N           INERT on the default path: the recursive filter\n"
        "                    ignores it. Kept for the old box-filter path\n"
        "\n"
        "SUB-PIXEL (on by default; worth 17 points of bad-1.0 on Middlebury v3)\n"
        "  --no-subpixel     integer disparities. A perfect integer answer scores\n"
        "                    45.6%% bad-1.0 where a perfect float one scores 0.8%%\n"
        "  --fit-parabola    the old estimator. The graded cost is locally a V, not\n"
        "                    a bowl, so the equiangular default is 0.7 points better\n"
        "\n"
        "INFERENCE\n"
        "  --iters N         MASDA message-passing iterations. 0 (the default) is\n"
        "                    winner-take-all under the same one-to-one constraint\n"
        "                    and measures BETTER while saving ~12 ms of CPU. The\n"
        "                    uniqueness and the margin are present either way\n"
        "  --topk N          candidates kept per pixel. 2 measured best; 0 keeps\n"
        "                    the whole volume                        (default 2)\n"
        "\n"
        "DIAGNOSTIC AND MEASUREMENT\n"
        "  --simd            NEON score kernel, AArch64 only, bit-identical: 1.6x\n"
        "                    on the score loop and 0.92x on the stage at D=60\n"
        "  --no-blockwise    keep the full cost volume at --topk 2. Slow, and NOT\n"
        ...
        "\n"
        "See doc/11-running.md for how to run this, and doc/09-matching.md for the\n"
        "measurement behind every default above.\n",
        argv[0]);
    return 2;
  }
